...
const SKDModal = (props) => {
  ...
  const onSearch = (page = 1) => {
    ...
    setPageIndex(page);
    // type：1收款单  checkStatus：1已审核  verifyStatus：-1待核销
    request({ url: '/api/accountF/list', method: 'GET', params: { page, pageSize, type: 1, checkStatus: 1, verifyStatus: -1, ...params, ...searchParams } })
      .then(res => {
        if (res && res.retcode == 0) {
  ...
  let columns = [
    { title: '源单编号', dataIndex: 'sn_self', align: 'center', width: 120 },
    {
      title: '业务类别',
      dataIndex: 'type',
      align: 'center',
    ...
    { title: '会员名称', dataIndex: 'organ_name', align: 'center', width: 100 },
    {
      title: '单据金额',
