...
const SupplierModal = (props) => {
  let { modalShow, onSelectedSupplier, modalClose, params = {} } = props;
  ...
  const onSearch = (page = 1) => {
    ...
    setPageIndex(page);
    request({ url: '/api/supplier/list', method: 'GET', params: { page, pageSize, status: 1, ...params, ...searchParams } })
      .then(res => {
        if (res && res.retcode == 0) {
          setSupplierList(res.page.list);
  ...
  let modalItems = [
    {
      ...
      content: <div>
        ...
        <MyTable
          ...
          dataSource={PublicService.transformArrayData(supplierList, true, true)}
          scroll={{ y: 475 }}
          rowKey='id'
        />
        <MyPagination
