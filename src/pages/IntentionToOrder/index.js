...
const IntentionToOrder = (props) => {
  ...
  let handleSaveData = (val) => {
    ...
    val.channelName = {1: '腾讯新闻', 2: '新浪', 3: '搜狐', 4: '百度百科', 5: '360百科', 6: '36氪等线上发稿渠道', 7: '百家号', 8: '今日头条', 9: '钛媒体', 10: '大鱼号', 11: '抖音', 12: '快手', 13: '小红书', 14: '视频号', 15: '公众号', 16: '其他'}[val.channelId];
  ...
  let searchItems = [
    ...(
      ...
      [{
        ...
        itemProps: {
          options: PublicService.transformProjectInfoToSelect(saleMgrList, 'value', 'label', true),
        },
      }] || []
    ),
    ...(
  ...
  let columns = [
    ...
    { title: '客户名称', dataIndex: 'nick_name', align: 'center', width: 150 },
    { title: '手机号', dataIndex: 'mobile', align: 'center', width: 150 },
    { title: '项目名称', dataIndex: 'project_name', align: 'center', width: 200 },
    {
      title: '是否成交',
      dataIndex: 'is_deal',
      align: 'center',
      ...
      render: (text, record) => (
        <>
          ...
          }
          {
            ...
            <>
              <Divider type='vertical' />
              <a onClick={onModalShow.bind(this, '创建材料单', record)}>创建材料单</a>
              <Divider type='vertical' />
              <a onClick={onModalShow.bind(this, '创建工程单', record)}>创建工程单</a>
            </>
          }
          {/* <Popconfirm
            title="是否删除这条数据?"
  ...
  if(userType == 'xiaoshou_mgr') {
    ...
    basicParams.salesId = assignedStatus; // 销售主管是否分配给销售人员 -1:已分配 -2:未分配
  }
  if(userType == 'xiaoshou') {
    basicParams.salesId = userId; // 查分配给自己的订单
    basicParams.memberId = assignedStatus; // 销售人员是否分配给会员 -1:已分配 -2:未分配
  }
  if(userType == 'huiyuan') {
