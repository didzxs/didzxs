...
const ProjectAcquisition = (props) => {
  ...
  const onSelectedGCD = (list) => {
    myForm && myForm.props.form.setFieldsValue({
      projName: list[0].proj_name,
      projSn: list[0].self_sn
    })
  }
  ...
  let columns = [
    ...
    { title: '单据编号', dataIndex: 'sn_self', align: 'center', width: 120, sorter: true },
    {
      ...
      width: 100,
      render: (text) => (
        {1: '盘亏', 2: '盘盈', 3: '样品', 4: '报废', 5: '其他', 6: '工程领料'}[text]
      )
    },
    ...(
  ...
  // 已审核、非仓库人员不可编辑
  let disabled = (modalType != '新增' && !!detail.checkStatus) || userType.indexOf('gongcheng') < 0 && userType.indexOf('gc') < 0;
  let modalColumns = [
    {
      title: '商品',
      dataIndex: 'prod_name',
  ...
  let modalItems = [
    ...
    ...(
      ...
      [{
        ...
        label: '',
        content: <div style={{marginTop: '-12px'}}>
          <Button onClick={onOtherModalShow.bind(this, setGCDModalShow)}>选择</Button>
        </div>,
        span: 3,
      }] || []
    ),
  ...
  let modalProps = {};
  if(userType != 'gongcheng' && userType.indexOf('gc') < 0 || detail.checkStatus) {
  ...
  if(userType == 'gc_weibao') {
    GCDParams.category = '1,2,3,6,8';
  }
  if(userType == 'gc_zhihuixiaofang') {
    GCDParams.category = '4';
  }
  if(userType == 'gc_gongcheng') {
    GCDParams.category = '5,7,9,10,11,12';
  }
  if(userType == 'gc_chengben') {
    GCDParams.category = '13,14';
  }

  let params = {
    searchItems,
    addBtnShow: userType.indexOf('gongcheng') > -1 || userType.indexOf('gc') > -1,
    columns,
    modalItems,
    editBtnShow: false,
    deleteBtnShow: false,
...
export default ProjectAcquisition;
